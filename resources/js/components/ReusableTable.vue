      ...
      <table class="min-w-full divide-y divide-gray-200 ">
        ...
        <tbody class="bg-white ">
          <template v-for="(item, index) in data" :key="index" :class="{ 'expanded': item && item.isExpanded }">
            <tr v-for="(item, index) in data" :key="index" @click="handleRowClick(item)" :class="[
...
.expand-leave-from {
  ...
  max-height: 500px;
  /* Start with a generous max-height */
  opacity: 1;
}
...
.expand-row td {
  padding: 0;
  /* Remove padding from the transition TD */
}
...
.expand-row>td>div {
  ...
  /* padding: 1.5rem; */
  /* Adjust as necessary based on StudentInformation.vue's slot content padding */
}
</style>