    ...
    <div class="grid grid-cols-2 gap-8">
      ...
      <div class="rounded-lg shadow-md p-6">
        ...
        <form @submit.prevent="submitForm" class="space-y-6">
          ...
          <div class="space-y-2">
            ...
            <input v-model="form.title_announcement" type="text"
              class="bg-[#ffff] p-2 text-[0.875rem] leading-[1.25rem] rounded-[0.5rem] border-2 w-full"
              placeholder="Enter announcement title">
            <p v-if="form.errors.title_announcement" class="text-[#FF0000] text-sm">{{ form.errors.title_announcement }}
            </p>
          </div>
          ...
          <div class="space-y-2">
            ...
            <textarea v-model="form.description_announcement" rows="4"
              class="bg-[#ffff] p-2 text-[0.875rem] leading-[1.25rem] rounded-[0.5rem] border-2 w-full"
              placeholder="Enter announcement description"></textarea>
            <p v-if="form.errors.description_announcement" class="text-[#FF0000] text-sm">{{
              form.errors.description_announcement }}</p>
          </div>

          <!-- Deadline -->
          <div class="space-y-2">
            <label class="block text-sm font-semibold">Deadline</label>
            <input v-model="form.deadline_announcement" type="datetime-local"
              class="bg-[#ffff] p-2 text-[0.875rem] leading-[1.25rem] rounded-[0.5rem] border-2 w-full">
            <p v-if="form.errors.deadline_announcement" class="text-[#FF0000] text-sm">{{
              form.errors.deadline_announcement }}</p>
          </div>
          ...
          <div class="flex justify-end space-x-4">
            ...
            <button type="submit"
              ...
              {{ form.processing ? (isEditMode ? 'Updating...' : 'Creating...') : (isEditMode ? 'Update Announcement' :
                'Create Announcement') }}
            </button>
          </div>
        </form>
      ...
      <div class="rounded-lg shadow-md p-6">
        ...
        <div class="flex items-center gap-4 mb-6">
          <select v-model="selectedYearLevelFilter"
            class="bg-[#ffff] p-2 text-[0.875rem] leading-[1.25rem] rounded-[0.5rem] border-2 border-gray-500">
            <option value="">All Year Levels</option>
            <option v-for="yearLevel in uniqueYearLevelsFilter" :key="yearLevel" :value="yearLevel">
              {{ yearLevel }}
            </option>
          </select>
          ...
          <select v-model="selectedSectionFilter"
            ...
            <option v-for="section in uniqueSectionsFilter" :key="section" :value="section">
              Section {{ section }}
            </option>
          </select>
        ...
        <div class="space-y-6 max-h-[600px] overflow-y-auto">
          ...
          <div v-for="announcement in filteredAnnouncements" :key="announcement.id"
            ...
            <div class="flex flex-wrap gap-2 mb-2">
              <!-- Year Levels -->
              <template v-if="announcement.is_all_year_levels">
                <span class="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-sm">
                  All year levels
                </span>
              </template>
              <template v-else>
                <span v-for="yearLevel in announcement.year_levels" :key="yearLevel"
              ...
              <!-- Sections -->
              <template v-if="announcement.is_all_sections">
                <span class="px-2 py-1 bg-green-100 text-green-800 rounded-full text-sm">
                  All sections
                </span>
              </template>
              <template v-else>
                <span v-for="section in announcement.sections" :key="section"
